...
b2Island.prototype = 
{
	...
	{
		var siGrav, world, worldGravNodes, bGravNodes, bc, gn, g, contactSolver, contactsOkay, jointsOkay, jointOkay, i, j, b;
		...
		for (i = 0; i < this.m_bodyCount; ++i)
		{
			...
			// add own gravity node's forces 
			// add global gravity node's forces
			// to body's self imposed gravity
			siGrav = b.m_selfImposedGravity.Copy();
			world = b.GetWorld();
			worldGravNodes = world.m_gravityNodes;
			bGravNodes = b.m_gravityNodes;
			bc = b.GetCenterPosition();
			
			// world grav nodes
			for(g = 0; g < worldGravNodes.length; g++) {
				gn = worldGravNodes[g];
				gn.GravityAddToOut(bc, siGrav);
			}
			
			// body grav nodes
			for(g = 0; g < bGravNodes.length; g++) {
				gn = bGravNodes[g];
				gn.GravityAddToOut(bc, siGrav);
			}
			
			b.m_linearVelocity.Add( b2Math.MulFV (step.dt, b2Math.AddVV(siGrav, b2Math.AddVV(gravity, b2Math.MulFV( b.m_invMass, b.m_force ) ) ) ) );
		...
		{
			this.m_joints[i].PrepareVelocitySolver(step);
		}
		...
		for (i = 0; i < this.m_jointCount; ++i) {
			...
			if(j.m_type === b2Joint.e_constantVolumeJoint) {
				...
				if(j.m_csHoldFlag < b2ConstantVolumeJoint.CS_HOLD_STEPS) {
					j.m_csHoldFlag++;
				}
				else {
					j.Split();
	...
	{
		var minSleepTime, linTolSqr, angTolSqr, i, b;
