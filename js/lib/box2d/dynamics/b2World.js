...
b2World.prototype = 
{
	initialize: function(worldAABB, gravity, doSleep, gravityNodes){
		var bd;
		...
		this.m_gravityNodes = (typeof gravityNodes !== 'undefined') ? gravityNodes : [];
	...
	// Get / Set gravity nodes
	// Gravity nodes are invisible areas that exert gravitational pull on nearby bodies
	// Pull is based on gravity node strength, and fall off outer/inner distances
	// Gravity node influence is calculated in b2Island's Solve function
	GetGravityNodes: function() {
		return this.m_gravityNodes;
	},
	SetGravityNodes: function(gravityNodes) {
		this.m_gravityNodes = gravityNodes;
	},
	AddGravityNode: function(gravityNode) {
		if(this.m_gravityNodes.indexOf(gravityNode) === -1) {
			this.m_gravityNodes[this.m_gravityNodes.length] = gravityNode;
		}
	},
	RemoveGravityNode: function(gravityNode) {
		var index = this.m_gravityNodes.indexOf(gravityNode);
		if(index !== -1) {
			this.m_gravityNodes.splice(index, 1);
		}
	},
	...
	CreateJoint: function(def){
		var j = b2Joint.Create(def, this.m_blockAllocator), b, i, dj, s;
		...
		// connect to bodies of constant volume joint
		if(j.m_type === b2Joint.e_constantVolumeJoint) {
			for(i = j.distanceJoints.length - 1; i >= 0; i--) {
				dj = j.distanceJoints[i];
				dj.m_body1.m_cvj = j;
				dj.m_body2.m_cvj = j;
			}
		}
	...
	{	
		var collideConnected, body1, body2, dj, s, i, b;
		...
		if(j.m_type === b2Joint.e_constantVolumeJoint) {
			for(i = j.distanceJoints.length - 1; i >= 0; i--) {
				dj = j.distanceJoints[i];
				dj.m_body1.m_cvj = null;
				dj.m_body2.m_cvj = null;
				this.DestroyJoint(dj);
			}
		}
	...
	Step: function(dt, iterations){
		...
		var island, c, stackSize, stack, k, seed, stackCount, cn, jn, response, b, other, j, i;
		...
		for (j = this.m_jointList; j !== null; j = j.m_next)
		{
		...
		//stack = (b2Body**)this.m_stackAllocator.Allocate(stackSize * sizeof(b2Body*));
		stack = [];
		...
		for (seed = this.m_bodyList; seed !== null; seed = seed.m_next)
		{
			...
			while (stackCount > 0)
			{
				...
				for (jn = b.m_jointList; jn !== null; jn = jn.next)
				{
	...
	Query: function(aabb, shapes, maxCount){
		...
		var results = [], count = this.m_broadPhase.QueryAABB(aabb, results, maxCount), i;
