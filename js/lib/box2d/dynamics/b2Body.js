...
b2Body.prototype = 
{
	...
	// Get gravity nodes (gravity forces only affecting this body and its shapes)
	GetGravityNodes: function() {
		return this.m_gravityNodes;
	},
	// adds an invisible point that exerts gravitational force on body towards magnet's position
	...
	// Set/Get self imposed gravity (gravity forces only affecting this body and its shapes)
	SetSelfImposedGravityV: function(v) {
		this.m_selfImposedGravity.SetV(v);
	},
	SetSelfImposedGravityXY: function(x, y) {
		this.m_selfImposedGravity.x = x;
		this.m_selfImposedGravity.y = y;
	},
	GetSelfImposedGravity: function() {
		return this.m_selfImposedGravity;
	},
	...
	initialize: function(bd, world){
		var i, sd, massData, massDatas, r, shape;
		...
		this.m_selfImposedGravity = bd.selfImposedGravity;
		this.m_gravityNodes = bd.gravityNodes;
	...
	IsConnected: function(other){
		var jn;
		for (jn = this.m_jointList; jn !== null; jn = jn.next)
		{
			if (jn.other === other) {
