...
{
	GetAnchor1: function(){
		var b1 = this.m_body1, tVec = new b2Vec2();
	...
	GetAnchor2: function(){
		var b2 = this.m_body2, tVec = new b2Vec2();
	...
	GetJointTranslation: function(){
		var b1 = this.m_body1, b2 = this.m_body2, tMat, r1X, r1Y, r2X, r2Y, p1X, p1Y, p2X, p2Y, dX, dY, ax1X, ax1Y, translation;
		...
		//d = b2Math.SubtractVV(p2, p1);
		dX = p2X - p1X;
		dY = p2Y - p1Y;
		//ax1 = b2Math.b2MulMV(b1.m_R, this.m_localXAxis1);
		tMat = b1.m_R;
		...
		//translation = b2Math.b2Dot(ax1, d);
		translation = ax1X*dX + ax1Y*dY;
		return translation;
	},
	GetJointSpeed: function(){
		var b1 = this.m_body1, b2 = this.m_body2, tMat, r1X, r1Y, r2X, r2Y, p1X, p1Y, p2X, p2Y, dX, dY, ax1X, ax1Y, v1, v2, w1, w2, speed;
		...
		//speed = b2Math.b2Dot(d, b2Math.b2CrossFV(w1, ax1)) + b2Math.b2Dot(ax1, b2Math.SubtractVV( b2Math.SubtractVV( b2Math.AddVV( v2 , b2Math.b2CrossFV(w2, r2)) , v1) , b2Math.b2CrossFV(w1, r1)));
		//b2D = (dX*(-w1 * ax1Y) + dY*(w1 * ax1X));
		//b2D2 = (ax1X * ((( v2.x + (-w2 * r2Y)) - v1.x) - (-w1 * r1Y)) + ax1Y * ((( v2.y + (w2 * r2X)) - v1.y) - (w1 * r1X)));
	...
	{
		var tImp = invTimeStep * this.m_limitImpulse, tMat, ax1X, ax1Y, ay1X, ay1Y;

		//ax1 = b2Math.b2MulMV(this.m_body1.m_R, this.m_localXAxis1);
		tMat = this.m_body1.m_R;
		ax1X = tImp * (tMat.col1.x * this.m_localXAxis1.x + tMat.col2.x * this.m_localXAxis1.y);
		ax1Y = tImp * (tMat.col1.y * this.m_localXAxis1.x + tMat.col2.y * this.m_localXAxis1.y);
		//ay1 = b2Math.b2MulMV(this.m_body1.m_R, this.m_localYAxis1);
		ay1X = tImp * (tMat.col1.x * this.m_localYAxis1.x + tMat.col2.x * this.m_localYAxis1.y);
		ay1Y = tImp * (tMat.col1.y * this.m_localYAxis1.x + tMat.col2.y * this.m_localYAxis1.y);
	...
	PrepareVelocitySolver: function(){
		var b1 = this.m_body1, b2 = this.m_body2, tMat, r1X, r1Y, r2X, r2Y, invMass1, invMass2, invI1, invI2, ay1X, ay1Y, eX, eY, ax1X, ax1Y, dX, dY, jointTranslation, P1X, P1Y, P2X, P2Y, L1, L2;
		...
		tMat = b1.m_R;
		ay1X = tMat.col1.x * this.m_localYAxis1.x + tMat.col2.x * this.m_localYAxis1.y;
		ay1Y = tMat.col1.y * this.m_localYAxis1.x + tMat.col2.y * this.m_localYAxis1.y;
		...
		if (b2World.s_enableWarmStarting)
		{
			...
			P1X = this.m_linearImpulse * this.m_linearJacobian.linear1.x + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.linear1.x;
			P1Y = this.m_linearImpulse * this.m_linearJacobian.linear1.y + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.linear1.y;
			...
			P2X = this.m_linearImpulse * this.m_linearJacobian.linear2.x + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.linear2.x;
			P2Y = this.m_linearImpulse * this.m_linearJacobian.linear2.y + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.linear2.y;
			...
			L1 = this.m_linearImpulse * this.m_linearJacobian.angular1 - this.m_angularImpulse + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.angular1;
			...
			L2 = this.m_linearImpulse * this.m_linearJacobian.angular2 + this.m_angularImpulse + (this.m_motorImpulse + this.m_limitImpulse) * this.m_motorJacobian.angular2;
	...
	SolveVelocityConstraints: function(step){
		var b1 = this.m_body1, b2 = this.m_body2, invMass1 = b1.m_invMass, invMass2 = b2.m_invMass, invI1 = b1.m_invI, invI2 = b2.m_invI, oldLimitImpulse, linearCdot, linearImpulse, angularCdot, angularImpulse, motorCdot, motorImpulse, oldMotorImpulse, limitCdot, limitImpulse;

		// Solve linear constraint.
		linearCdot = this.m_linearJacobian.Compute(b1.m_linearVelocity, b1.m_angularVelocity, b2.m_linearVelocity, b2.m_angularVelocity);
		linearImpulse = -this.m_linearMass * linearCdot;
		...
		angularImpulse = -this.m_angularMass * angularCdot;
	...
	SolvePositionConstraints: function(){
		var limitC, oldLimitImpulse, b1 = this.m_body1, b2 = this.m_body2, invMass1 = b1.m_invMass, invMass2 = b2.m_invMass, invI1 = b1.m_invI, invI2 = b2.m_invI, tMat, r1X, r1Y, r2X, r2Y, p1X, p1Y, p2X, p2Y, dX, dY, ay1X, ay1Y, linearC, linearImpulse, positionError, angularC, angularImpulse, angularError, ax1X, ax1Y, translation, limitImpulse;
		...
		linearImpulse = -this.m_linearMass * linearC;
