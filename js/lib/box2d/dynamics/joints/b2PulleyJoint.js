...
Object.extend(b2PulleyJoint.prototype, 
{
	...
	GetLength1: function(){
		var tMat, pX, pY, dX, dY;
		//b2Vec2 p = this.m_body1->m_position + b2Mul(this.m_body1->m_R, this.m_localAnchor1);
		tMat = this.m_body1.m_R;
		pX = this.m_body1.m_position.x + (tMat.col1.x * this.m_localAnchor1.x + tMat.col2.x * this.m_localAnchor1.y);
		pY = this.m_body1.m_position.y + (tMat.col1.y * this.m_localAnchor1.x + tMat.col2.y * this.m_localAnchor1.y);
		//b2Vec2 s = this.m_ground->m_position + this.m_groundAnchor1;
		//b2Vec2 d = p - s;
		dX = pX - (this.m_ground.m_position.x + this.m_groundAnchor1.x);
		dY = pY - (this.m_ground.m_position.y + this.m_groundAnchor1.y);
	...
	GetLength2: function(){
		...
		pX = this.m_body2.m_position.x + (tMat.col1.x * this.m_localAnchor2.x + tMat.col2.x * this.m_localAnchor2.y);
		pY = this.m_body2.m_position.y + (tMat.col1.y * this.m_localAnchor2.x + tMat.col2.y * this.m_localAnchor2.y);
		//b2Vec2 s = this.m_ground->m_position + this.m_groundAnchor2;
		//b2Vec2 d = p - s;
		dX = pX - (this.m_ground.m_position.x + this.m_groundAnchor2.x);
		dY = pY - (this.m_ground.m_position.y + this.m_groundAnchor2.y);
		return Math.sqrt(dX*dX + dY*dY);
	},
	...
	initialize: function(def){
		var tMat, tX, tY, d1Len, d2Len, length1, length2;
	...
	PrepareVelocitySolver: function(){
		var b1 = this.m_body1, b2 = this.m_body2, tMat, r1X, r1Y, r2X, r2Y, p1X, p1Y, p2X, p2Y, s1X, s1Y, s2X, s2Y, length1, length2, cr1u1, cr2u2, P1X, P1Y, P2X, P2Y;
		...
		s1X = this.m_ground.m_position.x + this.m_groundAnchor1.x;
		s1Y = this.m_ground.m_position.y + this.m_groundAnchor1.y;
		//b2Vec2 s2 = this.m_ground->m_position + this.m_groundAnchor2;
		s2X = this.m_ground.m_position.x + this.m_groundAnchor2.x;
		s2Y = this.m_ground.m_position.y + this.m_groundAnchor2.y;
		...
		//cr1u1 = b2Cross(r1, this.m_u1);
		cr1u1 = r1X * this.m_u1.y - r1Y * this.m_u1.x;
		//cr2u2 = b2Cross(r2, this.m_u2);
		...
		P1X = (-this.m_pulleyImpulse - this.m_limitImpulse1) * this.m_u1.x;
		P1Y = (-this.m_pulleyImpulse - this.m_limitImpulse1) * this.m_u1.y;
		//b2Vec2 P2 = (-this.m_ratio * this.m_pulleyImpulse - this.m_limitImpulse2) * this.m_u2;
		P2X = (-this.m_ratio * this.m_pulleyImpulse - this.m_limitImpulse2) * this.m_u2.x;
		P2Y = (-this.m_ratio * this.m_pulleyImpulse - this.m_limitImpulse2) * this.m_u2.y;
	...
	SolveVelocityConstraints: function(step){
		var b1 = this.m_body1, b2 = this.m_body2, tMat, r1X, r1Y, r2X, r2Y, v1X, v1Y, v2X, v2Y, P1X, P1Y, P2X, P2Y, Cdot, impulse, oldLimitImpulse;
		
		//r1 = b2Mul(b1.m_R, this.m_localAnchor1);
		tMat = b1.m_R;
		r1X = tMat.col1.x * this.m_localAnchor1.x + tMat.col2.x * this.m_localAnchor1.y;
		r1Y = tMat.col1.y * this.m_localAnchor1.x + tMat.col2.y * this.m_localAnchor1.y;
		//r2 = b2Mul(b2.m_R, this.m_localAnchor2);
		tMat = b2.m_R;
		r2X = tMat.col1.x * this.m_localAnchor2.x + tMat.col2.x * this.m_localAnchor2.y;
		r2Y = tMat.col1.y * this.m_localAnchor2.x + tMat.col2.y * this.m_localAnchor2.y;
	...
	SolvePositionConstraints: function(){
		var b1 = this.m_body1, b2 = this.m_body2, tMat, s1X, s1Y, s2X, s2Y, r1X, r1Y, r2X, r2Y, p1X, p1Y, p2X, p2Y, length1, length2, C, impulse, oldLimitPositionImpulse, linearError = 0.0;
