...
{
	var numOut = 0, vIn0 = vIn[0].v, vIn1 = vIn[1].v, 
...
{
	var vert1s = poly1.m_vertices, count2 = poly2.m_vertexCount, vert2s = poly2.m_vertices, normalX = poly1.m_normals[edge1].x, normalY = poly1.m_normals[edge1].y, tX = normalX, tMat = poly1.m_R, normalLocal2X, normalLocal2Y, vertexIndex2 = 0, minDot, tVec, dot, v1X, v1Y, v2X, v2Y, separation, i;
	...
	//normalLocal2 = b2Math.b2MulTMV(poly2.m_R, normal);
	...
	for (i = 0; i < count2; ++i)
	{
		//dot = b2Math.b2Dot(vert2s[i], normalLocal2);
		tVec = vert2s[i];
	...
	//separation = b2Math.b2Dot( b2Math.SubtractVV( v2, v1 ) , normal);
	v2X -= v1X;
	v2Y -= v1Y;
	//separation = b2Math.b2Dot( v2 , normal);
	separation = v2X * normalX + v2Y * normalY;
	return separation;
};
b2Collision.FindMaxSeparation = function(edgeIndex /*int ptr*/, poly1, poly2, conservative)
{
	var count1 = poly1.m_vertexCount, dX = poly2.m_position.x - poly1.m_position.x, dY = poly2.m_position.y - poly1.m_position.y, dLocal1X, dLocal1Y, edge, maxDot, dot, prevEdge, sPrev, nextEdge, sNext, bestEdge, bestSeparation, increment, i, s;

	// Vector pointing from the origin of poly1 to the origin of poly2.
	//var d = b2Math.SubtractVV( poly2.m_position, poly1.m_position );
	

	//dLocal1 = b2Math.b2MulTMV(poly1.m_R, d);
	...
	for (i = 0; i < count1; ++i)
	{
		//dot = b2Math.b2Dot(poly.m_normals[i], dLocal1);
...
{
	var count1 = poly1.m_vertexCount, vert1s = poly1.m_vertices, count2 = poly2.m_vertexCount, vert2s = poly2.m_vertices, vertex11 = edge1, vertex12 = edge1 + 1 === count1 ? 0 : edge1 + 1, tVec, normal1Local1X, normal1Local1Y, tX, invLength, normal1X, normal1Y, tMat, normal1Local2X, normal1Local2Y, vertex21, vertex22, minDot, normal2Local2X, normal2Local2Y, dot, tClip, i1, i2, i;

	// Get the normal of edge1.
	tVec = vert1s[vertex12];
	//normal1Local1 = b2Math.b2CrossVF( b2Math.SubtractVV( vert1s[vertex12], vert1s[vertex11] ), 1.0);
	...
	//normal1 = b2Math.b2MulMV(poly1.m_R, normal1Local1);
	...
	//normal1Local2 = b2Math.b2MulTMV(poly2.m_R, normal1);
	...
	for (i = 0; i < count2; ++i)
	{
		...
		//normal2Local2 = b2Math.b2CrossVF( b2Math.SubtractVV( vert2s[i2], vert2s[i1] ), 1.0);
		tVec = vert2s[i2];
		...
		//dot = b2Math.b2Dot(normal2Local2, normal1Local2);
...
{
	var edgeA, edgeAOut, separationA, edgeB, edgeBOut, separationB, poly1, poly2, edge1, flip, k_relativeTol, k_absoluteTol, incidentEdge, count1, vert1s, v11, v12, dvX, dvY, sideNormalX, sideNormalY, tX, tMat, invLength, frontNormalX, frontNormalY, v11X, v11Y, v12X, v12Y, frontOffset, sideOffset1, sideOffset2, clipPoints1, clipPoints2, np, pointCount, tVec, separation, cp, i;
	...
	//dv = b2Math.SubtractVV(v12, v11);
	dvX = v12.x - v11.x;
	dvY = v12.y - v11.y;

	//sideNormal = b2Math.b2MulMV(poly1.m_R, b2Math.SubtractVV(v12, v11));
	...
	//frontNormal = b2Math.b2CrossVF(sideNormal, 1.0);
	...
	//frontOffset = b2Math.b2Dot(frontNormal, v11);
	frontOffset = frontNormalX * v11X + frontNormalY * v11Y;
	//sideOffset1 = -b2Math.b2Dot(sideNormal, v11);
	sideOffset1 = -(sideNormalX * v11X + sideNormalY * v11Y);
	//sideOffset2 = b2Math.b2Dot(sideNormal, v12);
...
{
	var dX, dY, distSqr, radiusSum, separation, dist, a, tPoint;
	
	manifold.pointCount = 0;

	//d = b2Math.SubtractVV(circle2.m_position, circle1.m_position);
	dX = circle2.m_position.x - circle1.m_position.x;
	dY = circle2.m_position.y - circle1.m_position.y;
	//distSqr = b2Math.b2Dot(d, d);
	distSqr = dX * dX + dY * dY;
...
{
	var tPoint, dX, dY, xLocalX, xLocalY, tMat, tX, dist, normalIndex, separation, radius, tVec, vertIndex1, vertIndex2, eX, eY, length, u, pX, pY, i, s;
	
	manifold.pointCount = 0;

	// Compute circle position in the frame of the polygon.
	//xLocal = b2Math.b2MulTMV(poly.m_R, b2Math.SubtractVV(circle.m_position, poly.m_position));
	...
	{
		//s = b2Math.b2Dot(poly.m_normals[i], b2Math.SubtractVV(xLocal, poly.m_vertices[i]));
	...
	//e = b2Math.SubtractVV(poly.m_vertices[vertIndex2] , poly.m_vertices[vertIndex1]);
	...
	//u = b2Math.b2Dot(b2Math.SubtractVV(xLocal , poly.m_vertices[vertIndex1]) , e);
...
{
	var t1 = b.minVertex, t2 = a.maxVertex, d1X = t1.x - t2.x, d1Y = t1.y - t2.y, d2X, d2Y;
