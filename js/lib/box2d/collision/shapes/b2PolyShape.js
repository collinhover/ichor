...
Object.extend(b2PolyShape.prototype, 
{
	TestPoint: function(p){
		var pLocal, tVec, dot, i;
		
		//pLocal = b2Math.b2MulTMV(this.m_R, b2Math.SubtractVV(p, this.m_position));
		pLocal = new b2Vec2();
		pLocal.SetV(p);
		...
		{
			//dot = b2Math.b2Dot(this.m_normals[i], b2Math.SubtractVV(pLocal, this.m_vertices[i]));
			tVec = new b2Vec2();
			tVec.SetV(pLocal);
			tVec.Subtract(this.m_vertices[i]);

			dot = b2Math.b2Dot(this.m_normals[i], tVec);
			if (dot > 0.0)
			{
				return false;
			}
	...
	initialize: function(def, body, newOrigin){
		var hX, hY, tVec, aabb, localR, box, hcX, hcY, poly, centroidX, centroidY, uX, uY, length, minVertexX, minVertexY, maxVertexX, maxVertexY, v, i1, i2, positionX, positionY, broadPhase, i;
		...
		//position = this.m_position + b2Mul(this.m_R, this.m_localOBB.center);
	...
	Synchronize: function(position1, R1, position2, R2){
		var hX, hY, v1, v2, v3, v4, centerX, centerY, broadPhase;
	...
	ResetProxy: function(broadPhase){
		var proxy, R, absR, h, position, aabb;
		if (this.m_proxyId === b2Pair.b2_nullProxy)
		{
			return;
		}
	...
	{
		var dLocalX, dLocalY, bestIndex, bestValue, value, i;
