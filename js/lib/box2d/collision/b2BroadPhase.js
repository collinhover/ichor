...
//public:
	...
	CreateProxy: function(aabb, userData) {
		var index = 0, proxy, proxyId = this.m_freeProxy, boundCount, lowerValues = [], upperValues = [], axis, bounds, lowerIndex, upperIndex, lowerIndexOut, upperIndexOut, tArr, tEnd, tBound1, tBound2, tIndex, proxy2, i, j;
	...
	DestroyProxy: function(proxyId) {
		var proxy = this.m_proxyPool[ proxyId ], boundCount = 2 * this.m_proxyCount, axis, bounds, lowerIndex, upperIndex, lowerValue, upperValue, tArr, tEnd, tBound1, tBound2, tIndex, index, proxy2, index2, i, j; 
	...
	MoveProxy: function(proxyId, aabb) {
		var axis = 0, index = 0, bound, prevBound, nextBound, nextProxyId = 0, nextProxy, boundCount, proxy, newValues, oldValues, bounds, lowerIndex, upperIndex, lowerValue, upperValue, deltaLower, deltaUpper, prevProxyId, prevProxy;
	...
	QueryAABB: function(aabb, userData, maxCount) {
		var lowerValues = [], upperValues = [], lowerIndex = 0, upperIndex = 0, lowerIndexOut, upperIndexOut, count = 0, proxy, i;
		this.ComputeBounds(lowerValues, upperValues, aabb);
	...
	Validate: function() {
		var axis, bounds, boundCount, stabbingCount, bound, i;
...
//private:
	ComputeBounds: function(lowerValues, upperValues, aabb) {
		var minVertexX = aabb.minVertex.x, minVertexY = aabb.minVertex.y, maxVertexX, maxVertexY;
		
		//b2Settings.b2Assert(aabb.maxVertex.x > aabb.minVertex.x);
		//b2Settings.b2Assert(aabb.maxVertex.y > aabb.minVertex.y);

		//minVertex = b2Math.b2ClampV(aabb.minVertex, this.m_worldAABB.minVertex, this.m_worldAABB.maxVertex);
		...
		//maxVertex = b2Math.b2ClampV(aabb.maxVertex, this.m_worldAABB.minVertex, this.m_worldAABB.maxVertex);
	...
	TestOverlapValidate: function(p1, p2) {
		...
		for (axis = 0; axis < 2; ++axis)
		{
			...
			if (bounds[p1.lowerBounds[axis]].value > bounds[p2.upperBounds[axis]].value || bounds[p1.upperBounds[axis]].value < bounds[p2.lowerBounds[axis]].value) {
				return false;
			}
		}
	...
	TestOverlap: function(b, p) {
		...
		for (axis = 0; axis < 2; ++axis)
		{
			...
			if (b.lowerValues[axis] > bounds[p.upperBounds[axis]].value || b.upperValues[axis] < bounds[p.lowerBounds[axis]].value) {
				return false;
			}
		}
...
//public:
	...
	m_proxyPool: [], //b2Settings.b2_maxPairs),
	m_freeProxy: 0,

	m_bounds: [], //2*b2Settings.b2_maxProxies),

	m_queryResults: [], //b2Settings.b2_maxProxies),
